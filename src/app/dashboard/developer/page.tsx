...
function ActivityFeed() {
  ...
  const typeColor = (type: string) => {
    if (type === 'pay_in') return 'bg-[#effaf2] text-[#0f8b4b]';
    if (type === 'pay_out') return 'bg-blue-50 text-blue-600';
          ...
          {items.map((d) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {d.lastError && <p className="text-[11px] text-red-400 truncate">{d.lastError}</p>}
              </div>
...
export default function DeveloperPage() {
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { key: 'webhooks', label: 'Webhook Logs', icon: <Webhook size={13} /> },
  ];

  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-12 px-4 sm:px-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-[#081b10]">Developer</h1>
        <p className="text-xs text-[#45504b] mt-0.5">
          Monitor events, webhook deliveries, and platform activity
        </p>
      </div>
      ...
      <div className="bg-white border border-[#f0f4f1] rounded-2xl p-6 shadow-sm">
        ...
        <div className="flex gap-6 border-b border-[#eef2ef] mb-6">
          {tabs.map((t) => (
            <button
              ...
              className={cn(
                'pb-3 text-xs font-bold transition-all relative flex items-center gap-1.5',
                activeTab === t.key ? 'text-[#0f8b4b]' : 'text-[#667085] hover:text-[#081b10]',
              )}
            >
              {t.icon}
              {t.label}
        ...
        {activeTab === 'activity' ? <ActivityFeed /> : <WebhookLogs />}
      </div>
    </div>
  );
}
