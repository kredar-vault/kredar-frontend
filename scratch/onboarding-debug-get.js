const axios = require('axios');

async function run() {
  const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..clytGs6HDc_CUBOqPHxROZauk3vc072peELYx7buDP8";

  console.log("Fetching onboarding status using token...");
  try {
    const res = await axios.get('https://api.staging.kredar.xyz/api/v1/onboarding', {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });

    console.log("Onboarding Status Response Status:", res.status);
    console.log("Onboarding Status Response Body:", JSON.stringify(res.data, null, 2));
  } catch (err) {
    console.log("Error Response Status:", err.response ? err.response.status : 'No Response');
    console.log("Error Response Body:", err.response ? err.response.data : err.message);
  }
}

run();
